...
`).find(e=>e.startsWith(`data:`));if(t)try{r.push(JSON.parse(t.slice(5).trim()))}catch{}}return{events:r,remainder:n}}var s=`http://127.0.0.1:17861`,c=`navia_last_session_id`;async function ee(){return!!(await p({path:`/v1/health`})).ok}async function l(e){let t=g(await p({path:`/v1/sessions`,method:`POST`,headers:{"Content-Type":`application/json`},body:{client:`chrome-extension`,metadata:{source:e}}})).session_id;return await chrome.storage.local.set({[c]:t}),t}async function te(){let e=(await chrome.storage.local.get(c))[c];return typeof e==`string`&&e.startsWith(`sess_`)?e:null}async function u(){await chrome.storage.local.remove(c)}async function ne(e){let t=await p({path:`/v1/sessions/${e}`});return t.ok?g(t):(await u(),null)}async function d(e,t){let n=await p({path:`/v1/page/context`,method:`POST`,headers:{"Content-Type":`application/json`},body:{...e,session_id:t}});if(!n.ok)return{ok:!1,message:n.error?.message??`PageContext submit failed.`};let r=g(n);return await chrome.storage.local.set({[c]:t}),{ok:!0,pageId:r.page_id,structuredPage:r.structuredPage}}async function re(e,t,n,r){let i={path:`/v1/chat/stream`,method:`POST`,headers:{"Content-Type":`application/json`},body:{session_id:e,message:t,source:`typed`,request_id:`req_${crypto.randomUUID().replace(/-/g,``)}`,...r}};if(m())return h(i,n);let a=await fetch(`${s}/v1/chat/stream`,{method:i.method,headers:i.headers,body:JSON.stringify(i.body)});if(!a.body)throw Error(`SSE response body is empty.`);await f(a.body,n)}function ie(e){let t=new Map((e.artifacts??[]).map(e=>[e.turnId,e]));return(e.messages??[]).filter(e=>e.role===`user`||e.role===`assistant`||e.role===`system`).map(e=>({id:e.message_id,role:e.role,text:e.content,turnId:e.turn_id,artifact:e.turn_id?t.get(e.turn_id):void 0}))}async function f(e,t){let n=e.getReader(),r=new TextDecoder,i=``;for(;;){let{value:e,done:a}=await n.read();if(a)break;i+=r.decode(e,{stream:!0});let s=o(i);i=s.remainder;for(let e of s.events)await t(e)}}async function p(e){if(m()){let t=await chrome.runtime.sendMessage({type:`navia.runtimeFetch`,request:e});if(!t.ok||!t.response)throw Error(t.error??`Runtime proxy failed.`);return t.response.body}return await(await fetch(`${s}${e.path}`,{method:e.method??`GET`,headers:e.headers,body:e.body===void 0?void 0:JSON.stringify(e.body)})).json()}function m(){return typeof chrome<`u`&&!!chrome.runtime?.sendMessage&&window.location.protocol!==`chrome-extension:`}async function h(e,t){let n=chrome.runtime.connect({name:`navia.runtimeStream`}),r=``,i=Promise.resolve();return new Promise((a,s)=>{n.onMessage.addListener(e=>{if(e?.type===`chunk`&&typeof e.text==`string`){r+=e.text;let a=o(r);r=a.remainder,i=a.events.reduce((e,n)=>e.then(()=>t(n)),i),i.catch(e=>{n.disconnect(),s(e instanceof Error?e:Error(`Runtime stream event handling failed.`))});return}if(e?.type===`done`){i.then(()=>{n.disconnect(),a()}).catch(e=>{n.disconnect(),s(e instanceof Error?e:Error(`Runtime stream event handling failed.`))});return}e?.type===`error`&&(n.disconnect(),s(Error(String(e.message??`Runtime stream proxy failed.`))))}),n.postMessage({type:`navia.runtimeStream`,request:e})})}function g(e){if(!e.ok||e.data===null)throw Error(e.error?.message??`Runtime request failed.`);return e.data}var _=`navia-injected-host`,ae=440,v=`mermaid-renderer.html`;function oe(e,t,n=`push`){return a(e,t,n)}function y(e,t){return i(e,t)}function b(){return typeof chrome<`u`&&chrome.runtime?.getURL?chrome.runtime.getURL(v):v}function x(e,t=b()){let n=document.createElement(`div`);n.className=`navia-artifact navia-artifact-viewer`,n.dataset.rendered=`pending`,n.dataset.artifactId=e.artifactId;let r=document.createElement(`iframe`);r.className=`navia-mermaid-frame`,r.title=`Mermaid mindmap`,r.src=t,r.addEventListener(`load`,()=>{r.contentWindow?.postMessage({type:`navia.renderMermaid`,artifactId:e.artifactId,source:e.content},`*`)}),n.append(r);let i=t=>{let a=t.data;if(!(a.type!==`navia.mermaidRendered`||a.artifactId!==e.artifactId)&&!(t.source&&r.contentWindow&&t.source!==r.contentWindow&&!t.origin.startsWith(`chrome-extension://`))){if(n.dataset.rendered=a.status===`succeeded`?`true`:`false`,a.status!==`succeeded`){let e=a.message??`Mermaid render failed`;n.title=e,n.textContent=e}window.removeEventListener(`message`,i)}};return window.addEventListener(`message`,i),n}function S(){if(document.getElementById(_)||!/^https?:|^file:/.test(window.location.href))return null;let e=document.createElement(`div`);e.id=_,document.documentElement.appendChild(e);let n=e.attachShadow({mode:`open`}),r={open:!1,side:`right`,width:y(ae,window.innerWidth),activeTool:`chat`,runtimeStatus:`checking`,sessionId:null,pageContext:null,structuredPageDebug:null,pageSubmitted:!1,statusText:`正在检查 Runtime...`,streamStatus:`idle`,lastError:null},i=[{id:w(),role:`system`,text:`你可以直接提问。需要页面内容时，我会自动读取当前页面。`}],a=new Map,o=`push`,s=null;n.innerHTML=`${T()}${le()}`;let c=n.querySelector(`[data-testid='navia-frame']`),f=n.querySelector(`[data-testid='navia-ball']`),p=n.querySelector(`[data-testid='navia-hover-strip']`);n.querySelector(`[data-testid='navia-panel']`);let m=n.querySelector(`[data-testid='navia-resize-handle']`),h=n.querySelector(`[data-testid='navia-reconnect']`),g=n.querySelector(`[data-testid='navia-collapse']`),v=n.querySelector(`[data-testid='navia-debug-reconnect']`),b=n.querySelector(`[data-testid='navia-tool-chat']`),S=n.querySelector(`[data-testid='navia-debug-toggle']`),E=n.querySelector(`[data-testid='navia-read-page']`),D=n.querySelector(`[data-testid='navia-debug-read-page']`),O=n.querySelector(`[data-testid='navia-summary']`),k=n.querySelector(`[data-testid='navia-debug-summary']`),A=n.querySelector(`[data-testid='navia-mindmap']`),j=n.querySelector(`[data-testid='navia-debug-mindmap']`),M=n.querySelector(`[data-testid='navia-new-chat']`),N=n.querySelector(`[data-testid='navia-send']`),P=n.querySelector(`[data-testid='navia-input']`),F=n.querySelector(`[data-testid='navia-messages']`),I=n.querySelector(`[data-testid='navia-status']`),L=n.querySelector(`[data-testid='navia-page']`),R=n.querySelector(`[data-testid='navia-state-banner']`),z=n.querySelector(`[data-testid='navia-chat-notice']`),B=n.querySelector(`[data-testid='navia-structured-json']`);f.addEventListener(`click`,()=>H()),p.addEventListener(`click`,()=>H()),h.addEventListener(`click`,()=>W()),g.addEventListener(`click`,()=>U()),v.addEventListener(`click`,()=>W()),S.addEventListener(`click`,()=>ue(r.activeTool===`debug`?`chat`:`debug`)),E.addEventListener(`click`,()=>K()),D.addEventListener(`click`,()=>K()),O.addEventListener(`click`,()=>q(`总结这篇文章`)),k.addEventListener(`click`,()=>q(`总结这篇文章`)),A.addEventListener(`click`,()=>q(`生成 Mermaid 思维导图`)),j.addEventListener(`click`,()=>q(`生成 Mermaid 思维导图`)),M.addEventListener(`click`,()=>de()),N.addEventListener(`click`,()=>q(P.value));let V=()=>{let e=Math.max(44,P.value.split(`
`).length*20+12);P.style.height=`auto`,P.style.height=`${Math.min(Math.max(P.scrollHeight,e),120)}px`};return P.addEventListener(`input`,V),V(),P.addEventListener(`keydown`,e=>{(e.metaKey||e.ctrlKey)&&e.key===`Enter`&&(e.preventDefault(),q(P.value))}),pe(),window.addEventListener(`resize`,()=>{r.width=y(r.width,window.innerWidth),Z()}),W(),Z(),{open:H,close:U};function H(){if(r.open=!0,Z(),r.runtimeStatus===`online`&&!r.pageSubmitted){K();return}r.runtimeStatus!==`online`&&W().then(()=>{r.runtimeStatus===`online`&&!r.pageSubmitted&&K()})}function U(){r.open=!1,$(),Z()}function ue(e){r.activeTool=e,Z()}async function de(){r.sessionId=null,r.pageContext=null,r.structuredPageDebug=null,r.pageSubmitted=!1,r.streamStatus=`idle`,r.lastError=null,r.statusText=r.runtimeStatus===`online`?`已新建会话`:r.statusText,i.splice(0,i.length,{id:w(),role:`system`,text:`新会话已创建。你可以直接提问，页面相关问题会自动读取当前页面。`}),P.value=``,V(),r.runtimeStatus===`online`&&(r.sessionId=await l(`injected-panel`)),Z()}async function W(){r.runtimeStatus=`checking`,r.statusText=`正在检查 Runtime...`,r.lastError=null,Z();try{let e=await ee();r.runtimeStatus=e?`online`:`offline`,r.statusText=e?`Runtime online`:`Runtime offline`,e&&await fe()}catch{r.runtimeStatus=`offline`,r.statusText=`Runtime offline，请启动本地服务`,r.lastError=`Local Runtime 未连接，启动后点击重连。`}Z()}async function G(){if(r.sessionId)return r.sessionId;let e=await l(`injected-panel`);return r.sessionId=e,e}async function fe(){let e=await te();if(e)try{let t=await ne(e);if(!t)return;if(t.activePage&&!ce(t.activePage.url,window.location.href)){await u(),r.sessionId=null,r.pageContext=null,r.pageSubmitted=!1,r.statusText=`最近 session 与当前页面不匹配，请重新读取页面。`;return}r.sessionId=t.session_id,t.activePage&&(r.pageContext={url:t.activePage.url,title:t.activePage.title,domain:t.activePage.domain,captured_at:t.activePage.captured_at??new Date().toISOString(),headings:[],visible_text:``,cleaned_text:``},r.pageSubmitted=!0,r.statusText=`已恢复：${t.activePage.title}`);let n=ie(t);n.length>0&&i.splice(0,i.length,...n)}catch{r.statusText=`最近 session 恢复失败`}}async function K(){if(r.runtimeStatus!==`online`){r.statusText=`Runtime offline，无法提交页面上下文`,r.lastError=`Runtime offline，无法读取并提交当前页面。`,Z();return}r.statusText=`正在读取当前页面...`,r.lastError=null,Z();try{let e=t(document,window.location.href);r.pageContext=e;let n=await d(e,await G());r.pageSubmitted=n.ok,r.structuredPageDebug=n.structuredPage??null,r.statusText=n.ok?`已提交页面：${e.title}`:n.message??`页面上下文提交失败`}catch(e){r.pageSubmitted=!1,r.statusText=`读取失败：${e instanceof Error?e.message:`unknown error`}`,r.lastError=e instanceof Error?e.message:`PageContext 读取失败。`}Z()}async function q(e){let t=e.trim();if(!t)return;if(r.runtimeStatus!==`online`){Y(`system`,`Runtime offline，请启动 Local Runtime 后重试。`),r.lastError=`Runtime offline，无法发送消息。`,Z();return}if(!r.pageSubmitted&&C(t)&&(Y(`system`,`正在读取当前页面……`),await K(),!r.pageSubmitted)){Y(`system`,`当前页面无法读取，但你仍可以普通聊天。`),Z();return}let n=await G(),a=w();i.push({id:w(),role:`user`,text:t}),i.push({id:a,role:`assistant`,text:``}),P.value=``,r.streamStatus=`streaming`,r.lastError=null,Z();try{await re(n,t,e=>J(e,a)),r.streamStatus=`done`}catch(e){r.runtimeStatus=`offline`,r.streamStatus=`error`,r.lastError=`连接 Runtime 失败：${e instanceof Error?e.message:`unknown error`}`,X(a,r.lastError)}Z()}function J(e,t){switch(e.type){case`response.delta`:X(t,String(e.data.text??``));return;case`artifact.created`:X(t,``,e.data.artifact);return;case`tool.started`:r.streamStatus=`running ${String(e.data.tool_name??`tool`)}`,Z();return;case`tool.done`:r.streamStatus=`tool done`,Z();return;case`error`:r.lastError=String(e.data.message??e.data.code??`Runtime error`),X(t,`\n${r.lastError}`),r.streamStatus=`error`,Z();return;default:return}}function Y(e,t){i.push({id:w(),role:e,text:t}),Z()}function X(e,t,n){let r=i.find(t=>t.id===e);r&&(t&&(r.text+=t),n&&(r.artifact=n),Z())}function pe(){let e=!1;m.addEventListener(`pointerdown`,t=>{e=!0,m.setPointerCapture(t.pointerId),t.preventDefault()}),m.addEventListener(`pointermove`,t=>{e&&(r.width=y(r.side===`right`?window.innerWidth-t.clientX:t.clientX,window.innerWidth),Z())}),m.addEventListener(`pointerup`,()=>{e=!1})}function Z(){c.dataset.open=String(r.open),c.dataset.side=r.side,c.style.setProperty(`--navia-width`,`${r.width}px`),o=oe(r.width,window.innerWidth,o),c.dataset.mode=o,c.dataset.widthState=se(r.width,window.innerWidth,o),c.dataset.runtime=r.runtimeStatus,c.dataset.submitted=String(r.pageSubmitted),c.dataset.pageState=r.pageSubmitted?`submitted`:`missing`,c.dataset.stream=r.streamStatus,c.dataset.error=r.lastError?`true`:`false`,c.dataset.activeTool=r.activeTool,I.textContent=`${r.statusText} · ${r.streamStatus}`,L.textContent=r.pageContext?`${r.pageContext.title} · ${r.pageContext.domain} · headings ${r.pageContext.headings.length}`:`尚未读取页面`,B.textContent=JSON.stringify(ge(),null,2),R.textContent=me(),z&&(z.textContent=he()),b.classList.toggle(`active`,r.activeTool===`chat`),b.setAttribute(`aria-current`,r.activeTool===`chat`?`true`:`false`),S.setAttribute(`aria-pressed`,r.activeTool===`debug`?`true`:`false`),S.classList.toggle(`active`,r.activeTool===`debug`),n.querySelector(`[data-testid='navia-debug-pane']`)?.classList.toggle(`is-visible`,r.activeTool===`debug`),O.disabled=!Q(),k.disabled=!Q(),A.disabled=!Q(),j.disabled=!Q(),N.disabled=!Q();let e=F.scrollHeight-F.scrollTop-F.clientHeight<24;F.innerHTML=``;for(let e of i){let t=document.createElement(`article`);t.className=`navia-message ${e.role}`;let n=document.createElement(`pre`);n.textContent=e.text||(e.role===`assistant`?`...`:``),t.appendChild(n),F.appendChild(t),e.artifact?.metadata?.format===`mermaid`&&ve(e.artifact,t)}e&&(F.scrollTop=F.scrollHeight),r.open&&o===`push`&&_e(),(!r.open||o===`overlay`)&&$()}function Q(){return r.runtimeStatus===`online`&&r.streamStatus!==`streaming`}function me(){return r.lastError?r.lastError:r.runtimeStatus===`offline`?`Runtime offline，请启动本地服务后点击重连。`:r.runtimeStatus===`checking`?`正在检查 Local Runtime 状态。`:r.pageSubmitted?r.streamStatus===`streaming`?`正在生成回复，请稍候。`:r.streamStatus.startsWith(`running`)?`工具执行中：${r.streamStatus.replace(`running `,``)}`:r.statusText.startsWith(`已恢复`)?`已恢复最近会话，可继续基于当前页面提问。`:`当前页面已就绪，可进行摘要和问答。`:`你可以直接提问；需要页面内容时会自动读取当前页面。`}function he(){return r.runtimeStatus===`offline`?`Runtime offline，启动本地服务后可继续聊天。`:r.runtimeStatus===`checking`?`正在检查 Local Runtime。`:r.pageSubmitted?r.streamStatus===`streaming`?`正在生成回复。`:r.streamStatus.startsWith(`running`)?`正在执行工具：${r.streamStatus.replace(`running `,``)}`:`当前页面已就绪。`:`可直接聊天；页面相关问题会自动读取当前页面。`}function ge(){return r.structuredPageDebug?r.structuredPageDebug:{status:`missing`,message:`读取当前页面后，这里会显示 A 模块返回的 StructuredPageContext。`,expectedSignals:[`pageId`,`contentHash`,`metadata`,`headingTree`,`paragraphs`,`chunks`,`annotations`,`summaryDraft`]}}function _e(){if(!s){let e=document.documentElement,t={marginLeft:e.style.marginLeft,marginRight:e.style.marginRight,transition:e.style.transition};s=()=>{e.style.marginLeft=t.marginLeft,e.style.marginRight=t.marginRight,e.style.transition=t.transition,s=null}}let e=document.documentElement;e.style.transition=`margin 220ms ease`,r.side===`right`?(e.style.marginRight=`${r.width}px`,e.style.marginLeft=``):(e.style.marginLeft=`${r.width}px`,e.style.marginRight=``)}function $(){s&&s()}function ve(e,t){let n=a.get(e.artifactId);if(n&&n.content===e.content){t.appendChild(n.element);return}let r=x(e);a.set(e.artifactId,{content:e.content,element:r}),t.appendChild(r)}}function C(e){return/(总结|summary|summarize|mindmap|mermaid|思维导图|脑图|解释选区|解释选中|当前页面|这个页面|这页|这篇|这篇文章|这段|上面内容)/i.test(e)}function se(e,t,n){return t<900?`mobile`:n===`overlay`?`overlay`:e/t>=.48?`half`:`narrow`}function w(){return`ui_${crypto.randomUUID().replace(/-/g,``)}`}function ce(e,t){try{let n=new URL(e),r=new URL(t);return n.hash=``,r.hash=``,n.toString()===r.toString()}catch{return e===t}}function le(){return`
