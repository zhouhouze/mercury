...
产品名：Navia / 伴航  
角色名建议：小航  
阶段目标：Chrome 插件优先的 Headless 伴随式 AI MVP
...
Navia / 伴航是一个常驻在浏览器侧边栏的本地伴随式 AI 助手，能够理解当前网页，提供伴读问答、摘要生成、Mermaid 思维导图，并以可观测、可监督的 Headless AgentCore 为后续个人知识库、观影观赛陪伴、个人秘书与多端产品化打基础。
...
1. 可在 Chrome 中安装的 Side Panel Chatbox。
2. 当前网页上下文识别与提取。
3. 基于当前网页的摘要生成。
4. 基于当前网页的问答。
5. Mermaid 思维导图生成与前端预览。
6. 本地小参数模型实现用户意图识别。
7. Chrome Side Panel 支持基础文字对话，不依赖语音即可完成 V1 主流程。
8. 本地可微调模型用于思维导图生成。
...
- 网页内贴边悬浮球与站内嵌入面板体验。该方向来自 Mercury 远端 `PRD/窗口交互_PRD.md`，建议作为 V1.1 或后续前端体验增强阶段单独审计，不进入 V1.0-E 验收目标。
...
- 移动端同步。
- 默认读取本地文件。
- V1.0-E 不实现网页内悬浮球、挤压网页布局、覆盖式站内面板和 resize handle；这些能力归入 V1.x 体验增强路线。
...
作为用户，我打开一篇文章后，希望在侧边栏点击“总结”，AI 能基于当前网页生成结构化摘要，而不需要我复制粘贴正文。
...
- UI 通过 SSE 消费 `/v1/chat/stream`；Agent events 后续可复用 SSE 或扩展 WebSocket。
