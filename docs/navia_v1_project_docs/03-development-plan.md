...
模块内部任务统一使用 `MODULE_VERSIONING.md` 中定义的编号。项目级 `V1.2-A` 表示 V1.2 阶段里的 A 模块工作；A 模块内部能力使用 `A-V1.0-*` 编号。
...
- 明确模块内部编号规则，所有 A 模块新增任务使用 `A-V1.0-*`。
...
- A 模块内部编号 `A-V1.0-*` 已在 PRD、开发计划、验收计划和 stage gate 中同步。
