...
日期：2026-06-02
权威来源：`PRD/窗口交互_PRD.md`

---

## 1. 口径结论

从本版本起，所有与前端页面体验、插件入口、页面内面板、悬浮球、面板展开/收起、页面挤压、覆盖态和 resize 相关的 PRD 描述，全部以仓库根目录 `PRD/窗口交互_PRD.md` 为准。

`docs/navia_v1_project_docs` 中此前关于 Chrome Side Panel 作为 V1 主前端形态的描述全部降级为：
...
| 优先级 | 文档 | 适用范围 |
|---|---|---|
| P0 | `PRD/窗口交互_PRD.md` | 所有前端页面交互、插件入口、悬浮球、网页内面板、挤压/覆盖/resize 体验 |
| P0 | `docs/navia_v1_project_docs/06-api-contract.md` | Runtime API、SSE、事件、错误、工具合同 |
| P0 | `docs/navia_v1_project_docs/07-data-models.md` | Session、Turn、PageContext、Artifact、ToolCall、Event 数据模型 |
| P1 | `docs/navia_v1_project_docs/01-prd.md` | 产品路线、V1 能力范围、非目标 |
| P1 | `docs/navia_v1_project_docs/02-architecture.md` | 技术架构与模块边界 |
| P1 | `docs/navia_v1_project_docs/03-development-plan.md` | 分阶段开发计划 |
| P1 | `docs/navia_v1_project_docs/04-acceptance-plan.md` | 验收计划 |

如果文档之间出现冲突：

- 前端页面体验以 `PRD/窗口交互_PRD.md` 为准。
- Runtime / AgentCore / API / Event / Governance 以 `06-api-contract.md`、`07-data-models.md` 和合同冻结文档为准。
- 不允许用 Chrome Side Panel 验收替代 `PRD/窗口交互_PRD.md` 中的页面内交互验收。
...
替代原 Side Panel 主线。目标是实现 `PRD/窗口交互_PRD.md` 的页面内交互入口，并打通 PageContext。
...
```text
Go for revised planning. No implementation until V1.0-D/E/F stage-gates are rewritten against PRD/窗口交互_PRD.md.
```
