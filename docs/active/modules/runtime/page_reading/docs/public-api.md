...
A module planning and implementation items use `A-V1.x-*` numbering from `docs/active/project/MODULE_VERSIONING.md`. The current page perception stage is `A-V1.2`.
...
These fields are frozen as public A contracts through `docs/active/project/contracts/v1_2_adapter_contracts.md` and `docs/active/project/contracts/a_v1_2_page_perception.schema.json`. D/C may consume high-signal outputs as primary context only when `PagePerceptionQualityReport.downstreamReadiness = "pass"`. `degraded` is fallback / Debug evidence only; `fail` must fall back to `StructuredPageContext` or return `PAGE_CONTEXT_REQUIRED`.
